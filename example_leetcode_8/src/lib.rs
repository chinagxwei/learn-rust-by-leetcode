...
pub fn solve_sudoku_2(board: &mut Vec<Vec<char>>) {
    let (mut row, mut column) = (vec![(1 << 9) - 1; 9], vec![(1 << 9) - 1; 9]);
    let (mut ones, mut map) = (vec![0; 1 << 9], vec![0_i32; 1 << 9]);
    let mut cell = vec![vec![(1 << 9) - 1; 3]; 3];
    type M = Vec<i32>;
    type N<T> = Vec<Vec<T>>;
...
#[cfg(test)]
mod test {
    use crate::{is_valid_sudoku, solve_sudoku_1, solve_sudoku_2};
